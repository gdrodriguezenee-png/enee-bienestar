[control_memorando_2026_2.html](https://github.com/user-attachments/files/28728227/control_memorando_2026_2.html)[Uploadin<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Control de Documentos - Bienestar Social 2026</title>
...
.tab-btn[data-t="hoja"].active{background:#7a3a00;}
.tab-btn[data-t="dictamen"].active{background:#5a0080;}
.tab-btn[data-t="circular"].active{background:#006680;}
  ...
  <div class="login-card">
    ...
    <div class="lc"><label>Usuario</label>
      <select id="l-usuario">
        ...
        <option value="gustavo">Gustavo D. (Admin)</option>
        <option value="wendy">Wendy</option>
        <option value="oponce">O. Ponce</option>
        <option value="rosa">Rosa</option>
        <option value="adriana">Adriana</option>
        <option value="carmen">Carmen Caceres</option>
...
<div id="app">
  <header>
    ...
    <div class="h-title">
      <h1>Control de Documentos 2026</h1>
  ...
  <div class="toolbar">
    ...
    <div><span class="tlabel">Clausula</span>
      <select id="filtro-clausula">
        <option value="">Todas</option><option value="40">Clausula 40</option>
        <option value="55">Clausula 55</option><option value="57">Clausula 57</option>
        <option value="66">Clausula 66</option><option value="75">Clausula 75</option>
...
<div class="modal-overlay" id="modal-actividad">
  <div class="modal" style="width:780px;max-width:98vw;">
    ...
    <div style="margin-bottom:14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      <div>
        <span class="tlabel">Usuario</span>
        <select id="act-usuario" style="font-family:'Barlow',sans-serif;font-size:13px;padding:7px 11px;border:1.5px solid #dde3ed;border-radius:6px;background:#f0f3f8;outline:none;width:180px;">
          <option value="">Todos</option>
        </select>
      </div>
      <div>
        <span class="tlabel">Año</span>
        <select id="act-anio" style="font-family:'Barlow',sans-serif;font-size:13px;padding:7px 11px;border:1.5px solid #dde3ed;border-radius:6px;background:#f0f3f8;outline:none;width:120px;">
          <option value="">Todos</option>
        </select>
      </div>
      <div>
        <span class="tlabel">Seccion</span>
        <select id="act-tipo" style="font-family:'Barlow',sans-serif;font-size:13px;padding:7px 11px;border:1.5px solid #dde3ed;border-radius:6px;background:#f0f3f8;outline:none;width:180px;">
...
function cargarUsuarios(cb){
  apiGet('Usuarios', function(err, rows){
    ...
    // Si Sheets esta vacio, inicializar con la lista del codigo
    ...
    for(var i=0; i<rows.length; i++){
      ...
      // Si no hay tabs guardados, dar acceso a todo
    ...
    // Siempre usar la lista de Sheets aunque tenga 1 solo usuario
...
function guardarUsuariosEnSheets(){
  // Reescribir toda la hoja usuarios
  var filas = [];
  for(var i=0; i<USUARIOS.length; i++){
    var u=USUARIOS[i];
...
// Cargar usuarios desde Sheets al iniciar
...
function doLogin(){
  ...
  // Recargar usuarios desde Sheets para permisos frescos
  cargarUsuarios(function(){
    var u=null;
    for(var i=0;i<USUARIOS.length;i++){
...
function tiposPermitidos(){
  ...
  // Si tabs es string, convertir a array
...
function actualizarCabeceras(){
  ...
  var r='<tr>';
  if(esCirug){
    r+='<th>N</th><th>A Quien</th><th>Clave</th><th>Centro de Trabajo</th><th>Fecha</th><th>Hospital</th><th>Monto Cotizacion</th><th>Diagnostico</th><th>Acciones</th>';
  } else {
    r+='<th>N</th><th>A quien se remite</th><th>Realizado por</th><th>Fecha</th><th>Asunto</th>';
    if(esHoja) r+='<th>Fecha Actividad</th>';
    r+='<th>Acciones</th>';
  }
  th.innerHTML=r+'</tr>';
}
...
function apiPost(body,cb){
  fetch(SCRIPT_URL,{method:'POST',redirect:'follow',body:JSON.stringify(body)})
    .then(function(r){return r.text();})
    .then(function(txt){try{cb(null,JSON.parse(txt));}catch(e){cb(e);}})
    .catch(function(e){cb(e);});
}
...
function limpiarFecha(v){
  if(!v) return '';
  var s = String(v);
  // Si viene como objeto Date de Sheets: "2026-06-06T06:00:00.000Z"
  if(s.indexOf('T') !== -1) {
    var d = new Date(s);
    if(!isNaN(d)){
...
function obtenerSiguienteNo(tipo,cb){
  ...
  apiGet(tipo,function(err,rows){
    ...
    // Filtrar solo registros del año actual (columna 11 para std, 8 para cirug)
...
function render(){
  ...
  for(var i=0;i<datos.length;i++){
    var d=datos[i];
    var txt=(d.no+' '+d.remite+' '+d.resp+' '+d.fecha+' '+(d.asunto||'')+(d.diagnostico||'')).toLowerCase();
    if((!q||txt.indexOf(q)!==-1)&&(!resp||d.resp.trim()===resp)&&(!claus||(d.asunto||'').match(new RegExp('cl[aá]usula\\s+'+claus,'i'))))
      filtrados.push(d);
  }
  if(sortCol){
    ...
    // Sin orden activo: mostrar el ultimo registro primero
    filtrados.reverse();
  }
...
function abrirModal(idx){
  ...
  if(esCirug){
    ...
    document.getElementById('m-fecha').value=m?m.fecha:'';
...
function guardar(){
  ...
  if(editIdx===null){
    ...
    obtenerSiguienteNo(tipoActual,function(noFinal){
      ...
      apiPost(postData,function(err,res){
        if(!err&&res&&res.ok){
          ...
          registrarBitacora('Agregó', getTipo(tipoActual).label, noFinal, 'Nuevo registro');
        } else { toast('Error al guardar','error'); syncStatus('err','Error'); }
        hideLoad();
      });
    });
  } else {
    ...
    var editData={accion:'editar',tipo:tipoActual,no_original:orig.no,no:noVal,remite:remite,resp:resp,fecha:fecha,asunto:asunto,fact:fact,clave:clave,centro:centro,hospital:hospital,monto:monto,diagnostico:diagnostico};
    if(esCirugEdit) editData.cols=[noVal,remite,clave,centro,fecha,hospital,monto,diagnostico];
    apiPost(editData,function(err,res){
      if(!err&&res&&res.ok){
        datos[editIdx]={no:noVal,remite:remite,resp:resp,fecha:fecha,asunto:asunto,fact:fact,clave:clave,centro:centro,hospital:hospital,monto:monto,diagnostico:diagnostico,anio:anioActual,nueva:false};
        poblarResp(); render(); syncStatus('ok','Sincronizado'); toast('Registro N '+noVal+' actualizado');
        registrarBitacora('Editó', getTipo(tipoActual).label, noVal, 'Registro modificado');
...
function eliminar(idx){
  ...
  var t=getTipo(tipoActual);
  if(!confirm('Eliminar registro N '+d.no+' de '+t.label+'?')) return;
  showLoad('Eliminando...');
  apiPost({accion:'eliminar',tipo:tipoActual,no:d.no},function(err,res){
    if(!err&&res&&res.ok){
      ...
      registrarBitacora('Eliminó', getTipo(tipoActual).label, d.no, 'Registro eliminado');
...
function exportarCSV(){
  var t=getTipo(tipoActual);
  var csv='Tipo,N,A quien se remite,Realizado por,Fecha,Asunto,Fecha Actividad,Clave,Centro,Hospital,Monto,Diagnostico\n';
  var datos=DB[tipoActual]||[];
  for(var i=0;i<datos.length;i++){
    var d=datos[i];
    csv+='"'+t.label+'","'+d.no+'","'+(d.remite||'')+'","'+(d.resp||'')+'","'+(d.fecha||'')+'","'+(d.asunto||'')+'","'+(d.fact||'')+'","'+(d.clave||'')+'","'+(d.centro||'')+'","'+(d.hospital||'')+'","'+(d.monto||'')+'","'+(d.diagnostico||'')+'"\n';
  }
  var blob=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8;'});
  var a=document.createElement('a'); a.href=URL.createObjectURL(blob);
  a.download='control_'+tipoActual+'_2026.csv'; a.click();
}
...
function agregarUsuario(){
  ...
  USUARIOS.push({id:id,nombre:nombre,pass:pass,rol:rol});
  rebuildLoginSelect();
  renderUsuarios();
  guardarUsuariosEnSheets();
...
function cambiarPass(uid){
  for(var i=0;i<USUARIOS.length;i++){
    if(USUARIOS[i].id===uid){
      ...
      if(!nueva||nueva.trim().length<4){toast('Contrasena muy corta','warn');return;}
...
function abrirPermisos(uid){
  ...
  for(var j=0;j<TIPOS.length;j++){
    ...
    div.innerHTML += '<label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;'
      +'background:#f0f3f8;padding:8px 12px;border-radius:8px;border:1.5px solid #dde3ed;">'
      +'<input type="checkbox" value="'+t.id+'" '+checked+' style="width:16px;height:16px;cursor:pointer;">'
...
function guardarPermisos(){
  ...
  for(var j=0;j<USUARIOS.length;j++){
    if(USUARIOS[j].id===permisosUID){
      ...
      // Si el usuario afectado es el que esta en sesion, actualizar en tiempo real
      if(usuarioActual && usuarioActual.id===permisosUID){
        usuarioActual.tabs = tabs;
        construirTabs();
      }
      break;
    }
  ...
  guardarUsuariosEnSheets();
  toast('Permisos guardados. El usuario vera los cambios al iniciar sesion.','success');
  cerrarPermisos();
}
...
function registrarBitacora(accion, tipo, no, detalle){
  ...
  // Guardar en Sheets con cols para control total de columnas
  apiPost({
    accion: 'agregar',
    tipo:   'bitacora',
    cols:   [fecha, usuario, accion, tipo, String(no), detalle||'']
  }, function(err, res){
    if(err) console.log('Bitacora error:', err);
    else if(res && !res.ok) console.log('Bitacora fallo:', res.error);
  });
}
...
function actTab(tab){
  ...
  if(!isResumen) renderBitacora();
}

function renderBitacora(){
  var tbody=document.getElementById('bit-tbody');
  if(!BITACORA.length){
    tbody.innerHTML='<tr><td colspan="5" style="text-align:center;padding:20px;color:#8899aa;font-style:italic;">Sin modificaciones registradas.</td></tr>';
    return;
  }
  tbody.innerHTML='';
  BITACORA.forEach(function(b){
    var color = b.accion==='Eliminó'?'#fde0e0':b.accion==='Editó'?'#fff8e6':'#dff0e8';
    var tcolor = b.accion==='Eliminó'?'#b02020':b.accion==='Editó'?'#c07000':'#1a7a44';
    tbody.innerHTML+='<tr style="border-bottom:1px solid #c8d4e4;">'
      ...
      +'<td style="padding:8px 12px;"><span style="background:'+color+';color:'+tcolor+';font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:12px;padding:2px 8px;border-radius:20px;">'+b.accion+'</span></td>'
      +'<td style="padding:8px 12px;color:#8899aa;">'+b.tipo+'</td>'
      +'<td style="padding:8px 12px;text-align:center;font-family:Barlow Condensed,sans-serif;font-weight:700;color:#003366;">'+b.no+'</td>'
      +'</tr>';
  });
}
...
function cargarBitacora(cb){
  apiGet('bitacora', function(err, rows){
    ...
    BITACORA = [];
    // cols: [fecha, usuario, accion, tipo, no, detalle]
    ...
    // Mas reciente primero (ya vienen en orden de insercion)
    BITACORA.reverse();
    if(cb) cb();
  });
}
...
function abrirActividad(){
  ...
  TIPOS.forEach(function(t){
    (DB[t.id]||[]).forEach(function(d){
      if(d.resp&&resps.indexOf(d.resp.trim())===-1) resps.push(d.resp.trim());
    });
  });
  resps.sort();
...
function cerrarActividad(){
  document.getElementById('modal-actividad').classList.remove('show');
}
...
function renderActividad(){
  ...
  Object.keys(mapa).forEach(function(k){
    var r=mapa[k].resp;
    totales[r]=(totales[r]||0)+mapa[k].count;
  });
  ...
  Object.keys(totales).sort().forEach(function(r){
    cards.innerHTML+='<div style="background:#e8f0f8;border-radius:6px;padding:8px;text-align:center;border:1.5px solid #c8d4e4;">'
      ...
      +'<div style="font-size:10px;color:#8899aa;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+r+'</div>'
      +'</div>';
  });
  if(!Object.keys(totales).length){
    cards.innerHTML='<div style="color:#8899aa;font-style:italic;padding:10px;">Sin registros para los filtros seleccionados.</div>';
  }

  // Tabla detalle
  var tbody=document.getElementById('act-tbody');
  tbody.innerHTML='';
  var filas=Object.values(mapa).sort(function(a,b){
    if(a.resp!==b.resp) return a.resp.localeCompare(b.resp,'es');
  ...
  if(!filas.length){
    ...
    filas.forEach(function(f){
      tbody.innerHTML+='<tr style="border-bottom:1px solid #c8d4e4;">'
        ...
        +'<td style="padding:8px 12px;color:#8899aa;font-size:12px;">'+f.ultima+'</td>'
        +'</tr>';
    });
  }
}
...
g control_memorando_2026_2.html…]()
